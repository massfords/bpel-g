 ...
 * engine and the simulation engine. This is because most of the behavioral
 * differences are encapsulated in interfaces.
 */
public class AeBpelEngine extends AeAbstractServerEngine {
	...
	/**
	 * @see org.activebpel.rt.bpel.impl.IAeBusinessProcessEngineInternal#registerForCoordination(org.activebpel.wsio.receive.IAeMessageContext,
	 *      org.activebpel.rt.bpel.IAeBusinessProcess)
	 */
	public boolean registerForCoordination(IAeMessageContext aContext,
		...
		regRequest
				.setProtocolIdentifier(IAeSpCoordinating.AESP_PARTICIPANT_COMPLETION_PROTOCOL);
		regRequest.setProperty(IAeCoordinating.AE_COORD_PID,
				String.valueOf(aProcess.getProcessId()));
		IAeCoordinationManager mgr = getCoordinationManager();
		try {
			// register for coordination - assuming the subprocess is good to
			// go.
	...
	public void create() throws AeException {
		try {
			...
			throw new AeException(
					AeMessages.getString("AeEngineFactory.ERROR_6"), e); //$NON-NLS-1$
		}
	}
	 ...
	 * managers must do in order to do their jobs (e.g. init a db connection)
	 * should be done here in order to catch any fatal errors.
	 */
	...
	/**
	 * Starts the engine instances. Note this implementation just calls start.
	 */
	...
	public void start() throws AeBusinessProcessException {
		try {
			...
			throw new AeBusinessProcessException(
					AeMessages.getString("AeBpelEngine.ERROR_0"), e); //$NON-NLS-1$
		}
	}
	...
	/**
	 * Stops the engine. The engine will fault if it receives any inbound
	...
	/**
	 * Shut down the bpel engine. This shuts down managers used by the engine
	 * (e.g. timer and work).
	 */
	 ...
	 * 
	 * @see org.activebpel.rt.bpel.impl.AeBusinessProcessEngine#getWebServiceTimeout(org.activebpel.rt.bpel.impl.IAeProcessPlan,
	 *      java.lang.String)
	 */
	 ...
	 * 
	 * <p>
	 * <em>Note: This class is now declared as <code>static</code> and
	...
	private static class AeDispatchReceiveRunnableHelper extends AeAbstractWork
		 ...
		 * @param aBpelEngine
		 */
		private AeDispatchReceiveRunnableHelper(long aProcessId,
				int aLocationId, AeInboundReceive aInboundReceive,
				long aJournalId, AeBpelEngine aBpelEngine) {
		...
		public void run() {
			try {
				...
				// call dispatch using 'synchronous' flag since this is on a
				// worker thread.
				getBpelEngine().dispatchReceiveData(mProcessId, mLocationId,
						mInboundReceive, mJournalId, true /* runSynchronously */);
			} catch (Throwable t) {
				AeException.logError(t,
						AeMessages.format("AeBpelEngine.ERROR_4", mProcessId)); //$NON-NLS-1$
			}
		}
		 ...
		 */
		protected AeInboundReceive restoreInboundReceive()
				throws AeBusinessProcessException {
	...
	private static class AeProcessExecutionRunnableHelper extends
		 ...
		 * @param aBpelEngine
		 */
		private AeProcessExecutionRunnableHelper(long aProcessId,
				AeBpelEngine aBpelEngine) {
			mProcessId = aProcessId;
			mBpelEngine = aBpelEngine;
		}
		...
		public void run() {
			try {
				...
				AeException.logError(t,
						AeMessages.format("AeBpelEngine.ERROR_3", mProcessId)); //$NON-NLS-1$
			}
		}
	...
	/**
	 * Overrides method to handle multi-start processes due to the possibility
	 * of simultaneous arrival of start messsages.
	 * 
	 * @see org.activebpel.rt.bpel.impl.AeBusinessProcessEngine#createProcessWithMessage(org.activebpel.rt.bpel.impl.queue.AeInboundReceive,
	 *      org.activebpel.wsio.IAeMessageAcknowledgeCallback, boolean)
	 */
	protected long createProcessWithMessage(AeInboundReceive aInboundReceive,
			IAeMessageAcknowledgeCallback aAckCallback,
			boolean aQueueForExecution) throws AeBusinessProcessException {
		if (aInboundReceive.getProcessPlan().getProcessDef().isMultiStart()) {
			return doMultiStartCreateProcessWithMessage(aInboundReceive,
					aAckCallback, aQueueForExecution);
		} else {
			return super.createProcessWithMessage(aInboundReceive,
					aAckCallback, aQueueForExecution);
		}
	}
	 ...
	 * 
	 * @see org.activebpel.rt.bpel.impl.AeBusinessProcessEngine#internalCreateProcessWithMessage(org.activebpel.rt.bpel.impl.queue.AeInboundReceive,
	 *      org.activebpel.wsio.IAeMessageAcknowledgeCallback, long)
	 */
	protected IAeBusinessProcess internalCreateProcessWithMessage(
			AeInboundReceive aInboundReceive,
			IAeMessageAcknowledgeCallback aAckCallback, long aRestartProcessId)
			throws AeBusinessProcessException {
		...
		try {
			process = super.internalCreateProcessWithMessage(aInboundReceive,
					aAckCallback, aRestartProcessId);
	...
	/**
	 * Handles the special case of creating a multi-start process. Such a
	 * process can be created with the arrival of data for more than one
	 * activity. The spec requires these activities to be correlated with the
	 * same correlation sets. By the time we've reached this method, we've
	 * already determined that there is no existing process that is waiting for
	 * the receipt of this data and as such we've decided to create a new one.
	 * However, due to the concurrent nature of the engine, we need to
	 * synchronize here in order to prevent two processes from being created
	 * with the same correlated data.
	 * 
	 * @param aInboundReceive
	 * @param aAckCallback
	 */
	protected long doMultiStartCreateProcessWithMessage(
			AeInboundReceive aInboundReceive,
			IAeMessageAcknowledgeCallback aAckCallback,
			boolean aQueueForExecution) throws AeBusinessProcessException {
		long processId = IAeBusinessProcess.NULL_PROCESS_ID;
		AeMessageReceiver found = null;
		// Acquire a lock for this inbound receive so we don't create multiple
		// processes when two
		// correlated receives come in for a multi-start process.
		IAeLockManager.IAeLock lock = getLockManager().acquireLock(
				aInboundReceive.getProcessName(),
				calculateCorrelationHash(aInboundReceive.getCorrelation()), 60);
		try {
			found = getQueueManager().matchInboundReceive(aInboundReceive,
					false, aAckCallback);
			if (found == null) {
				// if nothing was in the queue, then we're sure we need to
				// create the process.
				// the standard create routine is executed here but since we're
				// still w/in the
				// sync block we'll hold up any other threads trying to create
				// the same type
				// of process. This will give our newly created process a chance
				// to execute
				// which in turn will have the multistart activities within it
				// execute and
				// queue themselves with their correlated data. Once we're out
				// of this sync
				// block, any threads we were holding up will have a chance to
				// check the queue
				// to see if there is a match now as a result of this create, or
				// they'll go on
				// to create their own process instances.
				processId = super.createProcessWithMessage(aInboundReceive,
						aAckCallback, aQueueForExecution);
			}
		} finally {
		...
		// I found something above in the queue but I didn't want to do the
		// handle within the sync block since
		// there's no point in holding up the creation of other processes of the
		// same type. Note that at this
		// point the receive has been consumed and is thus no longer in the
		// queue.
		if (found != null) {
			...
			dispatchReceiveData(processId, locationId, aInboundReceive,
					journalId, synchronous);
		}
		return processId;
	}

	/**
	 * Given a correlation map, this method calculates a hash value for the data
	 * in the map.
	 * 
	 * @param aCorrelationMap
	 *            The correlation map.
	...
	/**
	 * @see org.activebpel.rt.bpel.impl.AeBusinessProcessEngine#executeProcess(long,
	 *      boolean)
	 */
	protected void executeProcess(long aPid, boolean aReplyWaiting)
		...
		if (aReplyWaiting) {
			...
			IAeInputMessageWork work = new AeProcessExecutionRunnableHelper(
					aPid, this);
	 ...
	 * 
	 * @see org.activebpel.rt.bpel.impl.AeBusinessProcessEngine#dispatchReceiveData(long,
	 *      int, org.activebpel.rt.bpel.impl.queue.AeInboundReceive, long,
	 *      boolean)
	 */
	protected void dispatchReceiveData(long aProcessId, int aLocationId,
			AeInboundReceive aInboundReceive, long aJournalId,
			boolean aRunSynchronously) throws AeBusinessProcessException {
		// dispatch to the process either synchronously or asynchronously.
		if (aRunSynchronously) {
			internalDispatchReceiveData(aProcessId, aLocationId,
					aInboundReceive, aJournalId);
		} else {
			// invoke a worker thread, which in turn will call
			// dispatchReceiveData(..) (i.e. this same method) from the worker
			// thread with ** aRunSynchronously = true **
			IAeInputMessageWork work = new AeDispatchReceiveRunnableHelper(
					aProcessId, aLocationId, aInboundReceive, aJournalId, this);
	 ...
	 * 
	 * @see org.activebpel.rt.bpel.impl.AeBusinessProcessEngine#journalQueueInvokeData(long,
	...
	protected long journalQueueInvokeData(long aProcessId, int aLocationId,
		...
		AeTransactionManager.getInstance().begin();
		try {
			journalId = super.journalQueueInvokeData(aProcessId, aLocationId,
					aTransmissionId, aMessageData, aProperties, aAckCallback);
			AeTransactionManager.getInstance().commit();
		} catch (AeBusinessProcessException abe) {
			try {
				AeTransactionManager.getInstance().rollback();
	 ...
	 * Overrides method to journal and perform callback in a transaction.
	 * 
	 * @see org.activebpel.rt.bpel.impl.AeBusinessProcessEngine#journalQueueInvokeFault(long,
	...
	protected long journalQueueInvokeFault(long aProcessId, int aLocationId,
		...
		AeTransactionManager.getInstance().begin();
		try {
			journalId = super.journalQueueInvokeFault(aProcessId, aLocationId,
					aTransmissionId, aFault, aProperties, aAckCallback);
			AeTransactionManager.getInstance().commit();
		} catch (AeBusinessProcessException abe) {
			AeTransactionManager.getInstance().rollback();
			throw abe;
	...
	public IAeProcessPlan getProcessPlan(AeMessageContext aContext)
		...
		if (deploymentPlan == null) {
			Object[] args = { aContext.getProcessName().getNamespaceURI(),
					aContext.getProcessName().getLocalPart() };
	...
	/**
	 * Delegates the call to the deployment provider. Extracted the method to
	 * allow easy override by subclass
	 * 
	 * @param aContext
	 * @throws AeException
	 */
	protected IAeProcessDeployment getDeploymentByProcessName(
			AeMessageContext aContext) throws AeException {
		return AeEngineFactory.getBean(IAeDeploymentProvider.class)
				.findCurrentDeployment(aContext.getProcessName());
	}
	 ...
	 * @throws AeException
	 */
	protected AeRoutingInfo getRoutingInfo(AeMessageContext aContext)
			throws AeException {
		return AeEngineFactory.getBean(IAeDeploymentProvider.class)
				.getRoutingInfoByServiceName(aContext.getServiceName());
	}

	/**
	 * Validate that only the service name OR the process qname have been set on
	 * the message context.
	 * 
	 * @param aContext
	 */
	protected void validateMessageContext(IAeMessageContext aContext)
			throws AeBusinessProcessException {
		if (aContext.getServiceName() != null
				&& aContext.getProcessName() != null) {
			throw new AeBusinessProcessException(AeMessages.format(
					"AeBpelEngine.InvalidContext", //$NON-NLS-1$
					new Object[] { aContext.getServiceName(),
							aContext.getProcessName() }));
		}
	}
	 ...
	 * 
	 * @see org.activebpel.rt.bpel.impl.AeBusinessProcessEngine#queueReceiveDataInternal(org.activebpel.rt.bpel.impl.queue.AeInboundReceive,
	 *      org.activebpel.wsio.IAeMessageAcknowledgeCallback, boolean)
	 */
	protected long queueReceiveDataInternal(AeInboundReceive aInboundReceive,
			IAeMessageAcknowledgeCallback aAckCallback,
	...
	protected void visitManagers(IAeManagerVisitor aVisitor) throws Exception {
		for (IAeManager mgr : getManagers().values()) {
			mgr.accept(aVisitor);
		}
	}

	/**
	 * Visit the managers with the specified visitor. A convenience method for
	 * when the visitor is one that won't throw an exception.
	 * 
	 * @param aVisitor
	 */
	protected void visitManagersNoException(IAeManagerVisitor aVisitor) {
		try {
			...
			throw new RuntimeException(
					AeMessages.getString("AeBpelEngine.ERROR_7"), e); //$NON-NLS-1$
		}
	}
	 ...
	 * @throws AeBusinessProcessException
	 */
	protected IAePersistentProcessManager getPersistentProcessManager()
			throws AeBusinessProcessException {
		IAeProcessManager pm = getProcessManager();
		if (!(pm instanceof IAePersistentProcessManager)) {
			throw new AeBusinessProcessException(
					AeMessages
							.getString("AeBpelEngine.ERROR_NonPersistentProcessManager")); //$NON-NLS-1$
		}
	 ...
	 * @throws AeBusinessProcessException
	 */
	protected IAeJournalEntry getJournalEntry(long aJournalId)
			throws AeBusinessProcessException {
		IAeJournalEntry entry = getPersistentProcessManager().getJournalEntry(
				aJournalId);
		if (entry == null) {
			throw new AeBusinessProcessException(AeMessages.format(
					"AeBpelEngine.ERROR_MissingJournalEntry", aJournalId)); //$NON-NLS-1$
		}
	 ...
	 * @throws AeBusinessProcessException
	 */
	protected AeInboundReceive deserializeInboundReceive(long aProcessId,
			IAeJournalEntry aJournalEntry) throws AeBusinessProcessException {
		if (!(aJournalEntry instanceof AeInboundReceiveJournalEntry)) {
			Object[] params = { String.valueOf(aJournalEntry.getJournalId()),
					String.valueOf(aProcessId) };
			throw new AeBusinessProcessException(AeMessages.format(
					"AeBpelEngine.ERROR_NotJournaledInboundReceive", params)); //$NON-NLS-1$
		}
	...
	public boolean isRestartable(long aProcessId)
		...
		try {
			...
			return process.isSuspended()
					...
					&& ((IAePersistentProcessManager) getProcessManager())
							.isPersistent(aProcessId);
		} finally {
			// Always release the process.
			releaseProcess(process);
		}
	 ...
	 */
	public void restartProcess(long aProcessId)
			throws AeBusinessProcessException {
		IAeBusinessProcess process = getProcessById(aProcessId);
		...
		try {
			if (isRestartable(aProcessId)) {
				...
				AeInboundReceiveJournalEntry entry = getPersistentProcessManager()
						.getRestartProcessJournalEntry(aProcessId);

				if (entry == null) {
					throw new AeBusinessProcessException(
							AeMessages
									.format("AeBpelEngine.ERROR_MissingRestartProcessInboundReceive", String.valueOf(aProcessId))); //$NON-NLS-1$
				}

				// Deserialize the restart process inbound receive.
				AeInboundReceive inboundReceive = deserializeInboundReceive(
						aProcessId, entry);
				...
				// Now that nothing is queued, clear all existing journal
				// entries.
				...
				throw new AeBusinessProcessException(
						AeMessages
								.getString("AeBpelEngine.FailedToRestartErrorMessage")); //$NON-NLS-1$
			}
		} finally {
			// Always release the process.
			releaseProcess(process);

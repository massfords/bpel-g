...
public abstract class AeSOAPReceiveHandler extends AeAbstractReceiveHandler {
	/**
	 * Key for property which holds the AeBPELExtendedWSDLDef this service
	 * operates against
	 */
	public static final String WSDL_DEF_ENTRY = "org.activebpel.rt.axis.WsdlDefEntry"; //$NON-NLS-1$

	/**
	 * Key for property which specifies the AePartnerLinkDef which this service
	 * operates against
	 */
	public static final String PARTNER_LINK_ENTRY = "org.activebpel.rt.axis.PartnerLinkEntry"; //$NON-NLS-1$

	/**
	 * Key for property which holds the port type QName which service operates
	 * against
	 */
	...
	/**
	 * Checks that we have a single document with a root node of
	 * soapenv:Envelope
	 * 
	 * @see org.activebpel.rt.bpel.server.engine.receive.AeAbstractReceiveHandler#validateInputData(org.activebpel.rt.bpel.impl.IAeProcessPlan,
	 *      IAeExtendedMessageContext, org.w3c.dom.Document[])
	 */
	public void validateInputData(IAeProcessPlan aPlan,
			IAeExtendedMessageContext aContext, Document[] aDocArray)
			throws AeBusinessProcessException {
		if (aDocArray.length != 1) {
			throw new AeBusinessProcessException(
					AeMessages.getString("AeSOAPReceiveHandler.ERROR_1")); //$NON-NLS-1$
		}
		...
		if (!IAeConstants.SOAP_NAMESPACE_URI.equals(namespace)
				|| !"Envelope".equals(localname)) //$NON-NLS-1$
		{
			throw new AeBusinessProcessException(
					AeMessages.getString("AeSOAPReceiveHandler.ERROR_1")); //$NON-NLS-1$
		}
	...
	/**
	 * Checks that we have a single data object of type
	 ...
	 */
	public void validateInputData(IAeProcessPlan aPlan,
			IAeExtendedMessageContext aContext, IAeWebServiceMessageData aData)
			throws AeBusinessProcessException {
	 ...
	 * @throws AeBusinessProcessException
	 */
	protected void mapResponseAddressing(SOAPEnvelope aResponseEnv,
			IAeWsAddressingHeaders aInboundHeaders)
			throws AeBusinessProcessException {
		...
		if (aInboundHeaders.getReplyTo() != null) {
			IAeAddressingHeaders replyAddressing = AeEngineFactory.getBean(
					IAePartnerAddressing.class).getReplyAddressing(
					aInboundHeaders, aInboundHeaders.getAction());
			IAeAddressingSerializer ser = AeWsAddressingFactory.getInstance()
					.getSerializer(aInboundHeaders.getSourceNamespace());
			ser.serializeHeaders(replyAddressing, aResponseEnv);
		}
	}

	/**
	 * Creates a IAeWebServiceResponse from the BPEL engine response and a
	 * SOAPEnvelope
	 * 
	 * The returned response will replace the AeMessageData from the engine with
	 * one that contains the SOAPEnvelope with a type of soapenv:Envelope and
	 ...
	 * @return the response
	 */
	protected AeInvokeResponse createWsResponse(
			IAeWebServiceResponse aResponse, SOAPEnvelope aResponseEnv)
			throws AeBusinessProcessException {
		QName envName = new QName(aResponseEnv.getNamespaceURI(),
				aResponseEnv.getLocalName());
		AeWebServiceMessageData responseData = new AeWebServiceMessageData(
				envName);
		...
		if (aResponse != null) {
			...
			response.setBusinessProcessProperties(aResponse
	...
	/**
	 * Maps fault data from the BPEL engine to a SOAPEnvelope containing a Fault
	 * envelope.
	 * 
	 * The message data within the response will have a type of soapenv:Envelope
	 * and a single element containing the SOAPEnvelope document.
	 * 
	 * @param aContext
	 * @param aResponse
	 */
	public IAeWebServiceResponse mapFaultData(
			IAeExtendedMessageContext aContext, IAeWebServiceResponse aResponse)
			throws AeBusinessProcessException {
		try {
			...
			if (aResponse.getMessageData() != null) {
				...
				// Only convert the message data, not attachments, this is for
				// convenience
				// to get easy access to the first part
				// note we used to convert the whole message, but that destroys
				// attachments
				// see defect 3261, because once an attachment is read it is
				// closed and can't
				// be used again. Below we still pass on the original aResponse
				// so it needs
				// its attachments in good shape.
				// used to be this which overwrites attachments: IAeMessageData
				// data = AeDataConverter.convert(aResponse.getMessageData());
				IAeMessageData data = AeMessageDataFactory.instance()
						.createMessageData(
								aResponse.getMessageData().getMessageType(),
				...
				if (partData instanceof Document) {
					Element element = ((Document) partData)
							.getDocumentElement();
					detail.appendChild(detail.getOwnerDocument().importNode(
							element, true));
				} else {
					// The WS-I Basic Profile requires that fault messages'
					// single part
					// is defined as a schema element. If we get here then we're
					// going
					// to be replying with a non-compliant fault message.
					// Our fault routing logic extends the WS-I Basic Profile
					// spec to
					// allow for an xsi:type attribute to indicate the type.
					// This is
					// done to account for non-compliant web services.
					SOAPElement detailElement = detail
							.addChildElement(partName);
					detailElement.addTextNode(partData.toString());

					QName faultMessage = data.getMessageType();
					IAeProcessPlan plan = getProcessPlan(AeMessageContext
							.convert(aContext));
					AeBPELExtendedWSDLDef def = AeWSDLDefHelper
							.getWSDLDefinitionForMsg(plan, faultMessage);
					if (def != null) {
						Message wsdlMessage = def.getMessage(faultMessage);
						Part part = wsdlMessage.getPart(partName);
						// It would be strange indeed if the single part we
						// found was an element
						// and we were in this code. It's possible if they're
						// running with
						// validation turned off so in order to avoid a NPE,
						// I'll check for null.
						if (part.getTypeName() != null) {
							AeXmlUtil.declareXsiType(detailElement,
									part.getTypeName());
						}
					}
				}
	 ...
	 * @throws AeBusinessProcessException
	 */
	protected SOAPEnvelope getEnvelope(Document aDoc)
			throws AeBusinessProcessException {
		...
		try {
			...
			if (aDoc == null) {
				...
				String envString = AeXmlUtil.serialize(aDoc
						.getDocumentElement());
				msg = getMessageFactory().createMessage(null,
						new ByteArrayInputStream(envString.getBytes()));
			}

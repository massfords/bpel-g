...
public class AeToValidator extends AeBaseExpressionValidator {
        ...
        if (mDefAdapter == null && AeUtil.notNullOrEmpty(getDef().getQuery())) {
            mDefAdapter = new IAeExpressionDef() {
                ...
                public String getExpressionLanguage() {
                    // TODO (EPW) should create a different path for
                    // executing/analyzing/validating queries to be used in the
                    // future.
                    try {
                        return getValidationContext().getExpressionLanguageFactory().getBpelDefaultLanguage(
                                getBpelNamespace());
                    } catch (AeException e) {
                        // previous behavior of getBpelDefaultLanguage was to
                        // return null
                        return null;
                    }
                }
    ...
    public void validate() {
        if (getDef().getStrategyKey() == null) {
            ...
            // the most common error will be an unresolvable variable, check for
            // that and give custom message
        ...
        if (AeUtil.notNullOrEmpty(getDef().getVariable())) {
            AeVariableValidator variableValidator = getVariableValidator(getDef().getVariable(),
                    null, true, AeVariableValidator.VARIABLE_WRITE_TO_SPEC);
            if (variableValidator == null) {
                return;
            }

            // handles recording the usage based on the pattern of part, query,
            // property
            variableValidator.addUsage(this, getDef().getPart(), getDef().getQuery(),
                    getDef().getProperty());
        }

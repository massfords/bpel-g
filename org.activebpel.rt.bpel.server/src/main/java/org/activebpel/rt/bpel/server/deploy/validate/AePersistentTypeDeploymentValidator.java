...
/**
 * A deployment validator that checks to see if the deploypment persistence and
 * transaction type is non-default, then verifies that only permitted BPEL
 * constructs are included; as well as to verify that subprocess invokes are not
 * used.
 */

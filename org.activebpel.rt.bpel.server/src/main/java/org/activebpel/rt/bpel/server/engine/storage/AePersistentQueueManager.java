...
public class AePersistentQueueManager extends AeInMemoryQueueManager {
	...
	public void create() throws Exception {
		super.create();
		setStorage(getStorageFactory().getQueueStorage());
	}

	/**
	 * @see org.activebpel.rt.bpel.impl.AeBaseQueueManager#findMatchForInboundReceive(org.activebpel.rt.bpel.impl.queue.AeInboundReceive,
	 *      org.activebpel.wsio.IAeMessageAcknowledgeCallback)
	 */
	protected AeMessageReceiver findMatchForInboundReceive(
		...
		for (int tries = 0; tries < getDeadlockTryCount(); tries++) {
			try {
				return getStorage().findMatchingReceive(aInboundReceive,
						aAckCallback);
			} catch (AeStorageException ex) {
				// If the root cause is not a SQL exception, break out of the
				// retry loop.
				// TODO (EPW) We need some sort of AeStorageDeadlockException
				// here.
				if (!(ex.getCause() instanceof SQLException)) {
					...
					AeException
							.logError(
									null,
									AeMessages
											.getString("AePersistentQueueManager.INBOUND_RECEIVE_DEADLOCK_ERROR")); //$NON-NLS-1$
	...
	protected void addMessageReceiverInternal(AeMessageReceiver aMessageReceiver) {
		try {
			...
			AeException.logError(e,
					AeMessages.getString("AePersistentQueueManager.ERROR_0")); //$NON-NLS-1$
		}
	}

	/**
	 * @see org.activebpel.rt.bpel.impl.IAeQueueManager#removeMessageReceiver(long,
	 *      int)
	 */
	public boolean removeMessageReceiver(long aProcessId,
			int aMessageReceiverPathId) throws AeBusinessProcessException {
		try {
			return getStorage().removeReceiveObject(aProcessId,
					aMessageReceiverPathId) != null;
		} catch (AeException e) {
			throw new AeBusinessProcessException(
					AeMessages.getString("AePersistentQueueManager.ERROR_1"), e); //$NON-NLS-1$
		}
	}
	...
	public AeMessageReceiverListResult getMessageReceivers(
			AeMessageReceiverFilter aFilter) throws AeBusinessProcessException {
		return getStorage().getQueuedMessageReceivers(aFilter);
	}

	/**
	 * Called when the manager starts to do the initial loadup of saved alarms.
	 * These saved alarms were scheduled prior to stopping the manager. Now that
	 * it's started up again, the alarms must be loaded and scheduled.
	 */
	...
	/**
	 * @see org.activebpel.rt.bpel.impl.AeBaseQueueManager#scheduleAlarm(long,
	...
	public void scheduleAlarm(long aProcessId, int aLocationPathId,
			...
			throws AeBusinessProcessException {
		try {
			internalStoreAlarm(aProcessId, aLocationPathId, aGroupId, aAlarmId,
					aDeadline);
		} catch (AeStorageException e) {
			throw new AeBusinessProcessException(
					AeMessages.getString("AePersistentQueueManager.ERROR_0"), e); //$NON-NLS-1$
		}

		// Delegate the scheduling work to the in-memory alarm manager.
		internalScheduleAlarm(aProcessId, aLocationPathId, aGroupId, aAlarmId,
				aDeadline);
	}

	/**
	 * This method is called to store the alarm in whatever storage facility is
	 * being used. In the case of the simple persistent alarm manager, the alarm
	 * is stored in the configured storage object.
	 * 
	 * @param aProcessId
	 *            The process id of the alarm.
	 * @param aLocationPathId
	 *            The location path of the alarm.
	 * @param aGroupId
	 ...
	 */
	protected void internalStoreAlarm(long aProcessId, int aLocationPathId,
			int aGroupId, int aAlarmId, Date aDeadline)
			throws AeStorageException {
		getStorage().storeAlarm(aProcessId, aLocationPathId, aGroupId,
				aAlarmId, aDeadline);
	}

	/**
	 * @see org.activebpel.rt.bpel.impl.AeBaseQueueManager#internalRemoveAlarm(long,
	 *      int, int)
	 */
	protected boolean internalRemoveAlarm(long aProcessId, int aLocationPathId,
			int aAlarmId) throws AeBusinessProcessException {
		boolean removed = removeAlarmFromStore(aProcessId, aLocationPathId,
				aAlarmId);

		// Don't care if the super version of remove worked - the bottom line
		// is:
	...
	protected AePersistentProcessManager getProcessManager() {
		return (AePersistentProcessManager) AeEngineFactory.getEngine()
				.getProcessManager();
	}

	/**
	 * @see org.activebpel.rt.bpel.impl.AeBaseQueueManager#removeAlarmForDispatch(long,
	 *      int, int, int)
	 */
	public long removeAlarmForDispatch(long aProcessId, int aGroupId,
			int aLocationPathId, int aAlarmId)
			throws AeBusinessProcessException {
		AeBusinessProcessException rethrownException = null;

		for (int tries = 0; tries < getDeadlockTryCount(); tries++) {
			try {
				long journalId = getStorage().removeAlarmForDispatch(
						aProcessId, aGroupId, aLocationPathId, aAlarmId);
				super.internalRemoveAlarm(aProcessId, aLocationPathId, aAlarmId);
				return journalId;
			} catch (AeStorageException ex) {
				// If the root cause is not a SQL exception, break out of the
	 ...
	 */
	protected boolean removeAlarmFromStore(long aProcessId,
			int aLocationPathId, int aAlarmId)
			throws AeBusinessProcessException {
	...
	protected void loadAndScheduleAlarms() {
		try {
			...
			AeException.logError(e,
					AeMessages.getString("AePersistentQueueManager.ERROR_1")); //$NON-NLS-1$
		}
	}
	 ...
	 * 
	 * @param aAlarms
	 *            The list of alarms to schedule.
	 */
	...
	/**
	 * Does the work of actually scheduling an alarm for firing. This will
	 * delegate the work to the superclass, who keeps the list of currently
	 * scheduled alarms in memory and utilizes the timer service to handle the
	 * work of firing the alarm at the desired time.
	 * 
	 * @param aProcessId
	 *            The process id of the alarm.
	 * @param aLocationPathId
	 ...
	 * @throws AeBusinessProcessException
	 */
	protected void internalScheduleAlarm(long aProcessId, int aLocationPathId,
			int aGroupId, int aAlarmId, Date aDeadline)
			throws AeBusinessProcessException {
		super.scheduleAlarm(aProcessId, aLocationPathId, aGroupId, aAlarmId,
				aDeadline);
	}

	/**
	 * This method is called whenever the manager needs to schedule a set of
	 * previously persisted alarms (for example on startup).
	 * 

...
public class AeBpelFigureBase extends AeGraphFigure {
	...
	/**
	 * Indicates that the underlying BPEL activity has been executed or
	 * evaluated.
	 */
	...
	/**
	 * Adds the icon label component as a child of this figure. Subclasses
	 * should override this method with an empty block if it does not want to
	 * add and display a label. This method is called as part of the components
	...
	/**
	 * Returns the component that contains the bounding box (hit test area) for
	 * the anchor point. In general, this component is the icon label component
	...
	/**
	 * Returns the bounding box of the anchorpoint. In general this is the
	 * bounding box of the icon image of the activity.
	 * 
	...
	public Rectangle getAnchorBounds() {
		...
		if (getLabel() != null) {
			r.setLocation(getLabel().getLocation().x,
					getLabel().getLocation().y);
			if (getLabel().getIcon() != null) {
				r.setSize(getLabel().getIcon().getIconWidth(), getLabel()
						.getIcon().getIconHeight());
			} else {
				r.setSize(getLabel().getSize().width,
						getLabel().getSize().height);
			}
		} else {
			getBounds(r);

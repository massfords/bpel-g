 ...
 * <code>IAeStorage</code>. This implementation counts references to process
 * instances to decide when to persist a process to storage. This means that
 * every call to {@link #createBusinessProcess(IAeProcessPlan)} or
...
public class AePersistentProcessManager extends AeAbstractProcessManager
	...
	/**
	 * Process recovery object shared between {@link #prepareToStart} and
	 * {@link #start}.
	 */
	...
	protected void cancelReleaseTimer(long aProcessId) {
		...
		if (timer != null) {
			try {
				...
				// This should also never happen, but if it does, don't let it
				// keep
	 ...
	 */
	public IAeBusinessProcess createBusinessProcess(IAeProcessPlan aProcessPlan)
			throws AeBusinessProcessException {
		QName processName = aProcessPlan.getProcessDef().getQName();

		// Acquire the process wrapper with mutex on temporary id, to ensure we
		// have room for new process.
		AeProcessWrapper wrapper = getProcessWrapperWithMutex();

		// Generate new process instance in storage.
		IAeProcessDeployment deployment = AeProcessDeploymentFactory
				.getDeploymentForPlan(aProcessPlan);
	 ...
	 * @param aProcessId
	 * @param aDeployment
	 *            The deployment plan or <code>null</code>.
	 */
	protected IAeBusinessProcess createProcess(long aProcessId,
			IAeProcessDeployment aDeployment) throws AeBusinessProcessException {
		...
		IAeBusinessProcess process = AeDefToImplVisitor.createProcess(
				aProcessId, getEngine(), deployment);
		return process;
	}
	 ...
	 * <code>null</code> if there is no process wrapper for the given process
	 * id. Does not increment the wrapper's reference count or acquire the
	 * process mutex the way {@link #getProcessWrapperWithMutex(long)} does, so
	 * does not require a matching call to
	 * {@link #releaseProcessWrapperWithMutex(AeProcessWrapper)}. However, this
	 * means that successive calls to {@link #getCurrentProcessWrapper(long)}
	 * with the same process id may return different process wrappers.
	 * 
	 * @param aProcessId
	 */
	 ...
	 * @param aProcessId
	 */
	private IAeProcessDeployment getDeploymentPlan(long aProcessId)
			throws AeBusinessProcessException {
		...
		IAeProcessDeployment deployment = provider.findDeploymentPlan(
				aProcessId, processName);
		if (deployment == null) {
			throw new AeBusinessProcessException(AeMessages.format(
					"AePersistentProcessManager.ERROR_10", aProcessId)); //$NON-NLS-1$
		}
	 ...
	 */
	public IAeJournalEntry getJournalEntry(long aJournalId)
			throws AeBusinessProcessException {
	 ...
	 * @param aProcessId
	 * @param aForUpdate
	 *            <code>true</code> if the process should be persisted after it
	 *            is released from memory.
	 */
	protected IAeBusinessProcess getProcess(long aProcessId, boolean aForUpdate)
			throws AeBusinessProcessException {
		...
		try {
			...
			if (process == null) {
				...
				getEngine().fireMonitorEvent(
						IAeMonitorListener.MONITOR_PROCESS_LOADED,
						IAeMonitorListener.EVENT_DATA_PROCESS_LOAD_FROM_DB);
				...
				getEngine().fireMonitorEvent(
						IAeMonitorListener.MONITOR_PROCESS_LOADED,
						IAeMonitorListener.EVENT_DATA_PROCESS_LOAD_FROM_CACHE);
				if (aForUpdate)
			...
			getEngine().fireMonitorEvent(
					IAeMonitorListener.MONITOR_PROCESS_LOAD_TIME, loadTime);
		}
	}
	...
	public ProcessInstanceDetail getProcessInstanceDetails(long aProcessId) {
		// In order to return the correct instant detail snaphot (ie. avoid
		// discrepancies due to
		// latency when getting the data from the storage), first check to see
		// if the process is
		// in memory. If so, get the details from in memory process. Otherwise,
		// return the process
		...
		// value of wrapper.getProcess() may change from one call to the next.
		// In
		// other words, call wrapper.getProcess() once and hold onto the return
		// value.
		IAeBusinessProcess process = (wrapper != null) ? wrapper.getProcess()
				: null;
		if (process != null) {
	...
	/**
	 * Returns the process instance details using the process instance from the
	 * memory cache.
	 * 
	 * @param aProcess
	 * @return process instance details if available, or null otherwise.
	 */
	protected ProcessInstanceDetail getProcessInstanceDetailsFromMemory(
			IAeBusinessProcess aProcess) {
		ProcessInstanceDetail detail = null;
		try {
			detail = new ProcessInstanceDetail();
			...
			detail.setStarted(AeDate.toCal(aProcess.getStartDate()));
			detail.setEnded(AeDate.toCal(aProcess.getEndDate()));
		} catch (Exception e) {
			AeException
					.logError(
							e,
							AeMessages
									.format("AePersistentProcessManager.ERROR_6", aProcess.getProcessId())); //$NON-NLS-1$
		}
		return detail;
	}
	 ...
	 * @return process instance details if available, or null otherwise.
	 */
	protected ProcessInstanceDetail getProcessInstanceDetailsFromStorage(
			long aProcessId) {
		try {
			return getStorage().getProcessInstanceDetail(aProcessId);
		} catch (AeStorageException e) {
			AeException.logError(e, AeMessages.format(
					"AePersistentProcessManager.ERROR_6", aProcessId)); //$NON-NLS-1$
		}
		return null;
	}
	...
	/**
	 * Returns process wrapper for the a temporary process id to ensure that we
	 * have room for this process in memory. The real id is set afterwards.
	...
	protected AeProcessWrapper getProcessWrapperWithMutex(long aProcessId) {
		AeProcessWrapper wrapper = getProcessWrapperMap()
				.getWrapper(aProcessId);
		wrapper.acquireMutex();
		return wrapper;
	}

	/**
	 * Sets the process associated with the process wrapper through the map so
	 * the id an be updated
	 * 
	 * @param aWrapper
	 * @param aProcess
	 */
	protected void setProcessWrapperProcess(AeProcessWrapper aWrapper,
			IAeBusinessProcess aProcess) {
	...
	public ProcessList getProcesses(ProcessFilterType aFilter)
		...
		for (int tries = 0; true;) {
			try {
				...
				// try count.
				// TODO (EPW) We need to have a method isRetryableException on
				...
				if ((e.getCause() instanceof SQLException)
					...
					// backoff wait time
					// skew the max wait time by 5 so our range will be 0-128ms
					// up to 0-2048ms
					AeDbUtils
							.backOffWait(tries + 6, -1,
									"AePersistentProcessManager.ERROR_GetProcessesRetry"); //$NON-NLS-1$
				}
					...
					if (firstException != null) {
						AeException
								.logError(
										...
										AeMessages
												.getString("AePersistentProcessManager.ERROR_GetProcessesFirstException")); //$NON-NLS-1$
					}

					throw new AeBusinessProcessException(
							AeMessages
									.getString("AePersistentProcessManager.ERROR_7"), e); //$NON-NLS-1$
				}
			}
		}
	}
	...
	public int getProcessCount(ProcessFilterType aFilter)
		...
		try {
			...
			throw new AeBusinessProcessException(
					AeMessages.getString("AePersistentProcessManager.ERROR_7"), e); //$NON-NLS-1$
		}
	}
	...
	public long journalInboundReceive(long aProcessId, int aLocationId,
			AeInboundReceive aInboundReceive) throws AeBusinessProcessException {
		return getProcessStateWriter().journalInboundReceive(aProcessId,
				aLocationId, aInboundReceive);
	}
	...
	public void journalSentReply(long aProcessId, AeReply aSentReply,
		...
		long journalId = getProcessStateWriter().journalSentReply(aProcessId,
				aSentReply, aProcessProperties);
	...
	public void journalInvokeTransmitted(long aProcessId, int aLocationId,
		...
		long journalId = getProcessStateWriter().journalInvokeTransmitted(
				aProcessId, aLocationId, aTransmissionId);
		journalEntryDone(aProcessId, journalId);
	}
	...
	public long journalCompensateSubprocess(long aProcessId,
			String aCoordinationId) // throws AeBusinessProcessException
	{
		return getProcessStateWriter().journalCompensateSubprocess(aProcessId,
				aCoordinationId);
	}
	...
	public long journalInvokePending(long aProcessId, int aLocationId)
			throws AeBusinessProcessException {
		return getProcessStateWriter().journalInvokePending(aProcessId,
				aLocationId);
	}

	/**
	 * @see org.activebpel.rt.bpel.server.engine.IAeRecoverableProcessManager#journalEngineFailure(long,
	 *      int)
	 */
	public void journalEngineFailure(long aProcessId, int aDeadEngineId)
			throws AeBusinessProcessException {
		long journalId = getProcessStateWriter().journalEngineFailure(
				aProcessId, aDeadEngineId);
		journalEntryDone(aProcessId, journalId);
	}
	...
	public void notifyProcessWrapperMapFull() {
		...
		synchronized (getProcessTimerMap()) {
			// The process timers map is based on a LinkedHashMap, which
			// provides
			// insertion-order iterators. Use an iterator to find the oldest
			// timer
			// in the map.
	...
	/**
	 * Handles the case when we are about to drop our last reference to the
	 * specified process.
	 * 
	 * @param aWrapper
	 */
	protected void reallyReleaseProcess(AeProcessWrapper aWrapper) {
		if (aWrapper.isPersistent()) {
			try {
				// Soon we will have no more references to the process, so this
				// is
				...
				AeException
						.logError(
								e,
								AeMessages
										.format("AePersistentProcessManager.ERROR_8", aWrapper.getProcessId())); //$NON-NLS-1$
			}
		} else {
			IAeProcessLogger logger = AeEngineFactory
					.getBean(IAeProcessLogger.class);

			if (logger instanceof IAePersistentLogger) {
				IAeProcessLogEntry entry = ((IAePersistentLogger) logger)
						.getLogEntry(aWrapper.getProcessId());
				if (entry != null)
					entry.clearFromLog();
			}
		}
	...
	public void releaseProcess(IAeBusinessProcess aProcess) {
		...
		try {
			...
			if (wrapper.getCount() == 1) {
				...
				// non-persistent processes while they are running, so we get
				// here
			...
			//
			// If this is the last reference to the process and we didn't
			// schedule
	...
	/**
	 * Releases a process wrapper allocated by
	 * {@link #getProcessWrapperWithMutex(long)} and then calls
	 * {@link AeProcessWrapper#releaseMutex()}. If this is the last reference to
	 * the process, calls {@link #reallyReleaseProcess(AeProcessWrapper)} before
		...
		try {
			if ((aWrapper.getCount() == 1) && (aWrapper.getProcess() != null)) {
				try {
					...
					// The final release of a quick-release process means that
					// the
					// process is leaving memory for the first time since the
					// process
					// completed.
			...
			try {
				...
				// releasing the mutex. If we were to release the mutex first,
				// then
				// another thread blocked in getProcessWrapperWithMutex() could
				// conceivably make the following call to releaseWrapper()
				// before
	...
	public void removeProcess(long aProcessId)
		...
		try {
			...
			throw new AeBusinessProcessException(AeMessages.format(
					"AePersistentProcessManager.ERROR_1", aProcessId), e); //$NON-NLS-1$
		}
	}
	...
	public int removeProcesses(ProcessFilterType aFilter)
			throws AeBusinessProcessException {
		try {
			ProcessStateFilterValueType ps = aFilter.getProcessState();
			if (ps != ProcessStateFilterValueType.Completed
					&& ps != ProcessStateFilterValueType.CompletedOrFaulted
					&& ps != ProcessStateFilterValueType.Faulted) {
				// Only allow deleting of completed or faulted processes.
				aFilter.setProcessState(ProcessStateFilterValueType.CompletedOrFaulted);
			}
			...
			throw new AeBusinessProcessException(
					(message != null) ? message : AeMessages
							.getString("AePersistentProcessManager.ERROR_11"), e); //$NON-NLS-1$
		}
	}
	 ...
	 * @param aProcessId
	 */
	protected IAeBusinessProcess restoreProcess(long aProcessId)
			throws AeBusinessProcessException {
	 ...
	 * @param aWrapper
	 */
	protected void saveProcess(AeProcessWrapper aWrapper)
			throws AeBusinessProcessException {
		long millis = System.currentTimeMillis();
		...
		Set<Long> completedTransmissionIds = aWrapper
				.getCompletedTransmissionIds();
		...
		if (aWrapper.isModified() || !completedJournalIds.isEmpty()
				|| !completedTransmissionIds.isEmpty()
				|| (journalIdForRestart != 0)) {
	 ...
	 * @param aDelayMillis
	 */
	protected void scheduleReleaseTimer(AeProcessWrapper aWrapper,
			long aDelayMillis) {
		++mSaveAvertedCount;
	...
	public void setPlanManager(IAePlanManager aPlanManager) {
		if (aPlanManager instanceof IAeDeploymentProvider)
			...
			throw new IllegalArgumentException(
					AeMessages.getString("AePersistentProcessManager.ERROR_37")); //$NON-NLS-1$
	}
	...
	public void stop() {
		...
		synchronized (getRecoveryAndStopMutex()) {
			...
			// Print attribute value counts (when enabled in
			// AeProcessImplStateAttributeCounts).
	...
	public IAeBusinessProcess recreateBusinessProcess(long aProcessId,
			IAeProcessPlan aProcessPlan) throws AeBusinessProcessException {
		AeProcessWrapper wrapper = getProcessWrapperWithMutex(aProcessId);
	 ...
	 */
	public AeInboundReceiveJournalEntry getRestartProcessJournalEntry(
			long aProcessId) throws AeBusinessProcessException {
		AeProcessWrapper wrapper = getProcessWrapperWithMutex(aProcessId);
		...
		try {
			...
			// If the restart process journal id is still specified in the
			// process
			...
			if (restartProcessJournalId != IAeProcessManager.NULL_JOURNAL_ID) {
				entry = (AeInboundReceiveJournalEntry) getStorage()
						.getJournalEntry(restartProcessJournalId);
			}
			// Otherwise, the restart process journal entry has been set aside,
			// so
			// fetch it by type.
			else {
	 ...
	 */
	public void journalAllEntriesDone(long aProcessId)
			throws AeBusinessProcessException {
	...
	public long journalCoordinationQueueMessageReceived(long aProcessId,
		...
		return getProcessStateWriter().journalCoordinationQueueMessage(
				aProcessId, aMessage);
	}
	...
	public long journalCancelSubprocessCompensation(long aProcessId) {
		return getProcessStateWriter().journalCancelSubprocessCompensation(
				aProcessId);
	}
	...
	public long journalReleaseCompensationResources(long aProcessId) {
		return getProcessStateWriter().journalReleaseCompensationResources(
				aProcessId);
	}
	...
	public long journalNotifyCoordinatorsParticipantClosed(long aProcessId) {
		return getProcessStateWriter()
				.journalNotifyCoordinatorsParticipantClosed(aProcessId);
	}
	...
	public long journalCompensateCallback(long aProcessId,
		...
		return getProcessStateWriter().journalCompensateCallback(aProcessId,
				aLocationPath, aCoordinationId, aFault);
	}
	...
	public long journalCoordinatedActivityCompleted(long aProcessId,
			String aLocationPath, String aCoordinationId, IAeFault aFault) {
		return getProcessStateWriter().journalCoordinatedActivityCompleted(
				aProcessId, aLocationPath, aCoordinationId, aFault);
	}
	...
	public long journalDeregisterCoordination(long aProcessId,
		...
		return getProcessStateWriter().journalDeregisterCoordination(
				aProcessId, aLocationPath, aCoordinationId);
	}
	...
	protected class AeProcessReleaseTimerListener implements TimerListener {
		...
		/**
		 * Constructs timer listener to release the specified process from
		 * memory.
		 */
		...
		public void timerExpired(Timer aTimer) {
			...
			try {
				...
				AeException.logError(e, AeMessages.format(
						"AePersistentProcessManager.ERROR_50", getProcessId())); //$NON-NLS-1$
	...
	protected class AeProcessReleaseWork extends AeAbstractWork {
		...
		public void run() {
			try {
				...
				AeException.logError(t, AeMessages.format(
						"AePersistentProcessManager.ERROR_51", getProcessId())); //$NON-NLS-1$
			}
		}
	}

 ...
 */
public abstract class AeAbstractReceiveHandler implements IAeReceiveHandler,
	...
	/**
	 * @see org.activebpel.rt.bpel.impl.IAeReceiveHandler#handleReceiveData(org.activebpel.wsio.IAeWebServiceMessageData,
	 *      org.activebpel.wsio.receive.IAeMessageContext)
	 */
	public IAeWebServiceResponse handleReceiveData(
			IAeWebServiceMessageData aData, IAeMessageContext aContext)
			throws AeBusinessProcessException {
		AeExtendedMessageContext context = AeExtendedMessageContext
				.convertToExtended(aContext);
	...
	/**
	 * @see org.activebpel.rt.bpel.impl.IAeReceiveHandler#handleReceiveData(org.w3c.dom.Document[],
	 *      org.activebpel.wsio.receive.IAeMessageContext)
	 */
	public IAeWebServiceResponse handleReceiveData(Document[] aDocArray,
	 ...
	 * @param aContext
	 */
	protected void updateContextWithPlanInfo(IAeProcessPlan aPlan,
			AeExtendedMessageContext aContext)
			throws AeBusinessProcessException {
		if (AeUtil.isNullOrEmpty(aContext.getProcessName())
				&& AeUtil.isNullOrEmpty(aContext.getServiceName())) {
			aContext.setProcessName(aPlan.getProcessDef().getQName());
		}
	}

	/**
	 * Authorizes a service request based on the subject on the message context
	 ...
	 * @throws AeBusinessProcessException
	 */
	protected void authorizeRequest(IAeProcessPlan aPlan,
			AeExtendedMessageContext aContext)
			throws AeBusinessProcessException {
		// Get the subject
		...
		if (subject != null) {
			try {
				if (!getSecurityProvider().authorize(subject, aContext)) {
					String[] msgstr = { aContext.getPrincipal(),
							aContext.getOperation(), aContext.getServiceName() };
					throw new AeBusinessProcessException(AeMessages.format(
							"AeAbstractReceiveHandler.0", msgstr)); //$NON-NLS-1$
				}
	 ...
	 */
	protected IAeWebServiceResponse invokeProcessEngine(IAeProcessPlan aPlan,
			AeExtendedMessageContext aContext, IAeMessageData aData)
			throws AeBusinessProcessException {
		IAeAddressingHeaders wsa = AeAddressingHeaders.convert(aContext
		...
		if (durableInfo != null) {
			IAeReplyReceiver receiver = AeEngineFactory
					.getBean(IAeTransmissionTracker.class)
					.getDurableReplyFactory()
					.createReplyReceiver(
							IAeTransmissionTracker.NULL_TRANSREC_ID,
							durableInfo);
			AeEngineFactory.getEngine().queueReceiveData(aPlan, aData,
					receiver, aContext);
			return null;
		} else {
			IAeWebServiceResponse response = AeEngineFactory.getEngine()
					.queueReceiveData(aPlan, aContext, aData);
	 ...
	 * @throws AeBusinessProcessException
	 */
	protected IAeProcessPlan getProcessPlan(AeMessageContext aContext)
			throws AeBusinessProcessException {
	 ...
	 * @throws AeBusinessProcessException
	 */
	protected IAeProcessDeployment getDeploymentPlan(IAeProcessPlan aPlan)
			throws AeBusinessProcessException {
		IAeProcessDeployment deploymentPlan = AeEngineFactory.getBean(
				IAeDeploymentProvider.class).findCurrentDeployment(
				aPlan.getProcessDef().getQName());
	 ...
	 */
	protected AePartnerLinkDef getPartnerLinkDef(IAeProcessPlan aPlan,
	 ...
	 * @throws AeBusinessProcessException
	 */
	protected IAeEndpointReference getMyRoleEndpoint(IAeProcessPlan aPlan,
			IAeExtendedMessageContext aContext)
			throws AeBusinessProcessException {
		...
		try {
			...
			IAeEndpointReference myEndpoint = AeEngineFactory.getBean(
					IAePartnerAddressing.class).getMyRoleEndpoint(deployment,
			...
			if (!AeUtil.isNullOrEmpty(aContext.getServiceName())) {
				myEndpoint.setServiceName(new QName(def.getMyRolePortType()
						.getNamespaceURI(), aContext.getServiceName()));
			...
			if (headers != null) {
				...
				{
					// add elements to message context to be passedthrough as
					// headers on subsequent invokes
	 ...
	 */
	protected QName getMyRolePortTypeQName(IAeProcessPlan aPlan,
			IAeMessageContext aContext) {
		AePartnerLinkDef plDef = aPlan.getProcessDef().findPartnerLink(
				aContext.getPartnerLink());
		AePartnerLinkDefKey plDefKey = new AePartnerLinkDefKey(plDef);

		return aPlan.getMyRolePortType(plDefKey);
	}

	public IAeSecurityProvider getSecurityProvider() {
		return mSecurityProvider;
	}

	public void setSecurityProvider(IAeSecurityProvider aSecurityProvider) {
		mSecurityProvider = aSecurityProvider;
	}
}

 ...
 */
public abstract class AeWSIOInvokeHandler implements IAeInvokeHandler,
		IAePolicyConstants {
	...
	/**
	 * Sets up the context for an invoke, resolving WSDL definitions for the
	 * service and SOAP binding operation, resolves policy references, and
	 * ws-addressing for the target endpoint.
	 * 
	 * @param aContext
	 ...
	 */
	protected void setupInvokeContext(AeInvokeContext aContext,
			IAeInvoke aInvokeQueueObject, String aQueryData)
			throws AeBusinessProcessException {
		aContext.setInvoke(aInvokeQueueObject);

		AeAddressHandlingType addressHandling = AeAddressHandlingType
				.getByName(aQueryData);
		...
		AeBPELExtendedWSDLDef wsdlDef = getWsdlDef(aInvokeQueueObject,
				endpointReference);
		Service wsdlService = getServiceObject(wsdlDef, endpointReference);
		aContext.setService(wsdlService);

		// The service name can be null in some circumstances. Specifically, if
		// the addressHandling type is
		// URN or URL, then we'll attempt to use the address found in the
		// wsa:Address
		// as opposed to the soap:address in the service.
		if (wsdlService == null
				&& addressHandling == AeAddressHandlingType.SERVICE)
			throw new AeBusinessProcessException(
					AeMessages.getString("AeInvokeHandler.ERROR_0")); //$NON-NLS-1$

		PortType portType = getPortType(wsdlService, endpointReference,
				aInvokeQueueObject);
		Operation operation = portType.getOperation(
				aInvokeQueueObject.getOperation(), null, null);
		aContext.setOperation(operation);
		if (operation == null)
			throw new AeBusinessProcessException(
					AeMessages
							.format("AeInvokeHandler.ERROR_MISSING_OPERATION", aInvokeQueueObject.getOperation())); //$NON-NLS-1$
		...
		String url = getEndpointUrl(wsdlService, endpointReference,
				aContext.getAddressType());
		endpointReference.setAddress(url);
		String soapAction = getSoapAction(wsdlService, operation.getName(),
				endpointReference);
		...
		if (isWsaMandatory(endpointReference, aContext.getReplyTo())) {
			...
			IAeAddressingHeaders wsaHeaders = new AeAddressingHeaders(
					endpointReference.getSourceNamespace());
			wsaHeaders.setAction(soapAction);
			wsaHeaders.setTo(url);
			wsaHeaders.setReplyTo(aContext.getReplyTo());
			IAePartnerAddressing pa = AeEngineFactory
					.getBean(IAePartnerAddressing.class);
			IAeEndpointReference newEndpoint = pa.updateEndpointHeaders(
					wsaHeaders, endpointReference);
		...
		if (wsdlService != null) {
			...
			BindingOperation bop = getBindingOperation(wsdlService,
					operation.getName(), endpointReference);
	...
	/**
	 * Extract the wsa:ReplyTo <code>IAeWebServiceEndpointReference</code> from
	 * the <code>IAeInvoke</code> object and see if we need to include engine
	 * managed correlation headers.
	 * 
	 * @param aInvoke
	 */
	protected IAeEndpointReference updateReplyToEndpoint(IAeInvoke aInvoke)
			throws AeBusinessProcessException {
	 ...
	 * @param aEndpointRef
	 */
	protected Service getServiceObject(AeBPELExtendedWSDLDef aDef,
			IAeWebServiceEndpointReference aEndpointRef)
			throws AeBusinessProcessException {
		...
		if (aEndpointRef.getServiceName() != null) {
			...
			// we should generate a warning message.
			// we may still be able to do the invoke but w/o the binding info we
	...
	/**
	 * Mandatory WSA information should be added automatically if 1. we have a
	 * ReplyTo address from the partner endpoint reference properties --OR-- 2.
	 * we have headers required for engine managed correlation in the default
	 * reply to ref props TODO: Use the 2006 WSA SOAP Binding spec to decide
	 * this in the future
	 */
	protected boolean isWsaMandatory(
			IAeWebServiceEndpointReference aPartnerEndpoint,
			IAeWebServiceEndpointReference aMyEndpoint)
			throws AeBusinessProcessException {
		try {
			IAeAddressingHeaders wsaHeaders = new AeAddressingHeaders(
					aPartnerEndpoint.getSourceNamespace());
			wsaHeaders.setReferenceProperties(aPartnerEndpoint
					.getReferenceProperties());
			return (wsaHeaders.getReplyTo() != null || AeUtil
					.notNullOrEmpty(aMyEndpoint.getReferenceProperties()));
	 ...
	 * @throws AeBusinessProcessException
	 */
	protected String getEndpointUrl(Service aWsdlService,
			IAeWebServiceEndpointReference aEndpointRef,
			AeAddressHandlingType aAddressHandlingType)
			throws AeBusinessProcessException {
		...
		if (aAddressHandlingType == AeAddressHandlingType.SERVICE) {
			...
			if (port == null) {
				Object[] args = { aEndpointRef.getServiceName(),
						aEndpointRef.getServicePort() };
				throw new AeBusinessProcessException(AeMessages.format(
						"AeInvokeHandler.MissingPort", args)); //$NON-NLS-1$
			}
		...
		// in either case, send the url through the urn mapping facility to see
		// if
	 ...
	 * @throws AeBusinessProcessException
	 */
	protected String getRequestStyle(Service aWsdlService,
			String aOperationName, IAeWebServiceEndpointReference aEndpointRef)
			throws AeBusinessProcessException {
		...
		// find out if document or rpc and get the soap action from
		// binding/operation
		BindingOperation bop = getBindingOperation(aWsdlService,
				aOperationName, aEndpointRef);
	 ...
	 * @throws AeBusinessProcessException
	 */
	protected String getSoapAction(Service aWsdlService, String aOperationName,
			IAeWebServiceEndpointReference aEndpointRef)
			throws AeBusinessProcessException {
		...
		if (aWsdlService != null) {
			...
			for (int i = 0; extList != null && i < extList.size(); i++) {
				...
				if (obj instanceof UnknownExtensibilityElement) {
					...
					if ("operation".equals(sop.getElement().getLocalName())) //$NON-NLS-1$
					{
						soapAction = sop.getElement()
								.getAttribute("soapAction"); //$NON-NLS-1$
						break;
					}
				}
			}
	 ...
	 * @throws AeBusinessProcessException
	 */
	protected BindingOperation getBindingOperation(Service aWsdlService,
			String aOperationName, IAeWebServiceEndpointReference aEndpointRef)
			throws AeBusinessProcessException {
		Port port = aWsdlService.getPort(aEndpointRef.getServicePort());
		Binding binding = port.getBinding();
		...
		if (binding == null
				|| (operation = binding.getBindingOperation(aOperationName,
						null, null)) == null) {
			Object[] args = { aEndpointRef.getServiceName(),
					aEndpointRef.getServicePort() };
			throw new AeBusinessProcessException(AeMessages.format(
					"AeInvokeHandler.MissingBinding", args)); //$NON-NLS-1$
		}
		return operation;
	}
	 ...
	 * @throws AeBusinessProcessException
	 */
	protected PortType getPortType(Service aWsdlService,
			IAeWebServiceEndpointReference aEndpointRef, IAeInvoke aInvoker)
			throws AeBusinessProcessException {
		...
		if (aWsdlService != null) {
			...
			if (port == null) {
				Object[] args = {
						aEndpointRef.getServiceName().getNamespaceURI(),
						aEndpointRef.getServiceName().getLocalPart(),
						aEndpointRef.getServicePort() };
				throw new AeBusinessProcessException(AeMessages.format(
						"AeInvokeHandler.MissingPort", args)); //$NON-NLS-1$
			}
			if (port.getBinding() == null) {
				throw new AeBusinessProcessException(
						AeMessages
								.format("AeInvokeHandler.MissingBinding", aEndpointRef.getServicePort())); //$NON-NLS-1$
			}
			portType = port.getBinding().getPortType();
		} else {
			// TODO (RN) May want to revisit this to pass the
			// IAeProcessDeployment instead of looking it up
			IAeContextWSDLProvider wsdlProvider = AeEngineFactory.getBean(
					IAeDeploymentProvider.class).findDeploymentPlan(
					aInvoker.getProcessId(), aInvoker.getProcessName());
			if (wsdlProvider != null) {
				AeBPELExtendedWSDLDef def = AeWSDLDefHelper
						.getWSDLDefinitionForPortType(wsdlProvider,
								aInvoker.getPortType());
				if (def != null)
					portType = def.getPortType(aInvoker.getPortType());
				else
					throw new AeBusinessProcessException(
							AeMessages
									.format("AeInvokeHandler.NoDefForPortType", aInvoker.getPortType())); //$NON-NLS-1$
			}
		}
	 ...
	 */
	protected AeWebServiceMessageData extractMessageData(Fault aWsdlFault,
			Element aFirstDetailElement) {
		AeWebServiceMessageData data = new AeWebServiceMessageData(aWsdlFault
				.getMessage().getQName());
		String partName = (String) aWsdlFault.getMessage().getParts().keySet()
				.iterator().next();
		if (isSimpleType(aFirstDetailElement)) {
			data.setData(partName, AeXmlUtil.getText(aFirstDetailElement));
		} else {
			Document doc = AeXmlUtil.newDocument();
			Element details = (Element) doc.importNode(aFirstDetailElement,
					true);
	...
	/**
	 * Helper method that checks to see if the passed elements data is a simple
	 * type or complex type.
	 * 
	 * @param aElement
	 *            The element to check the contents of.
	 */
	protected boolean isSimpleType(Element aElement) {
		boolean simple = false;
		// TODO Simple check for now, a complex type will have attributes and/or
		// child elements.
		if (AeUtil.isNullOrEmpty(aElement.getNamespaceURI())
				&& AeXmlUtil.getFirstSubElement(aElement) == null) {
			simple = true;
	...
	/**
	 * Gets all of the policy attachments from the endpoint and wsdl definitions
	 * that may be required for the invoke
	 * 
	 * @param aDef
	 * @param aInvoke
	 * @param aEndpoint
	...
	protected List<Element> getWsdlPolicies(AeBPELExtendedWSDLDef aDef,
		...
		return AeWSDLPolicyHelper.getEffectiveOperationPolicies(wsdlProvider,
				aEndpoint, aInvoke.getPortType(), aInvoke.getOperation(), null);
	}

	/**
	 * Gets the WSDL def for an invoke.
	 * 
	 * If the endpoint includes a service name, we'll lookup the wsdl for the
	 * service. If no service, we'll lookup the wsdl for the invoke's portType
	 * 
	 * @param aInvoke
	 * @param aEndpointRef
	 * @throws AeBusinessProcessException
	 */
	protected AeBPELExtendedWSDLDef getWsdlDef(IAeInvoke aInvoke,
			IAeWebServiceEndpointReference aEndpointRef)
			throws AeBusinessProcessException {
		AeBPELExtendedWSDLDef def = null;
		if (aEndpointRef.getServiceName() != null) {
			// they have a service name, see if we can find the wsdl for it
			def = AeWSDLDefHelper.getWSDLDefinitionForService(
					AeEngineFactory.getBean(IAeCatalog.class),
					aEndpointRef.getServiceName());
			...
			if (def == null) {
				...
				if (wsdlProvider != null)
					def = AeWSDLDefHelper.getWSDLDefinitionForService(
							wsdlProvider, aEndpointRef.getServiceName());
			}
		} else {
			IAeContextWSDLProvider wsdlProvider = AeEngineFactory.getBean(
					IAeDeploymentProvider.class).findDeploymentPlan(
					aInvoke.getProcessId(), aInvoke.getProcessName());
			if (wsdlProvider != null)
				def = AeWSDLDefHelper.getWSDLDefinitionForPortType(
	...
	/**
	 * If the process initiator mapping header policy is in effect, we will add
	 * a header as a reference property to the endpoint.
	 * 
	 ...
	 * @throws AeBusinessProcessException
	 */
	protected void handlePrincipalHeader(IAeInvoke aInvoke,
			AeInvokeContext aContext) throws AeBusinessProcessException {
		// check for principal header policy
		QName principalName = (QName) aContext.getCallProperties().get(
				IAePolicyConstants.TAG_ASSERT_MAP_PROCESS_INTIATOR);
		if (!AeUtil.isNullOrEmpty(principalName)) {
			...
			if (principalHeader == null) {
				...
				principalHeader = AeXmlUtil.createElement(
						principalName.getNamespaceURI(),
						"abx", principalName.getLocalPart()); //$NON-NLS-1$

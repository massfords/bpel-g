 ...
 * the database calls to an instance of IAeQueueStorageProvider. The purpose of
 * this class is to encapsulate storage 'logic' so that it can be shared across
 * multiple storage implementations (such as SQL and Tamino).
 */
public class AeQueueStorage extends AeAbstractStorage implements
		IAeQueueStorage, PreferenceChangeListener {
	/** A flag indicating whether to log hash collisions. */
	private boolean mLogCollisions;

	private int mMaxCorrelationCombinations;

	public AeQueueStorage() {
		init();
	}

	protected void init() {
		mMaxCorrelationCombinations = AePreferences
				.getMaxCorrelationCombinations();
	}

	/**
	 * Convenience method to get the storage provider cast to a queue storage
	 * provider.
	 */
	...
	/**
	 * Gets the max number of correlation combinations to produce when routing
	 * an inbound receive w/ correlation data to IMA's with join style
	 * correlations
	 */
	...
	/**
	 * @see org.activebpel.rt.bpel.server.engine.storage.IAeQueueStorage#findMatchingReceive(org.activebpel.rt.bpel.impl.queue.AeInboundReceive,
	 *      org.activebpel.wsio.IAeMessageAcknowledgeCallback)
	 */
	public AeMessageReceiver findMatchingReceive(
		...
		while (!done) {
			// If the inbound receive is correlated, look for an exact match
			// first.
			if (aInboundReceive.isCorrelated()) {
				AeCorrelationCombinations combos = aInboundReceive
						.getProcessPlan()
						.getProcessDef()
						.getCorrelationProperties(
								aInboundReceive.getPartnerLinkOperationKey());

				// combos could be null if the correlationSets were all
				// initiate="yes". In this case, none of the message
				// receivers would have been queued with a hash for correlated
				// properties.
				if (combos != null) {
					AeCorrelationCombinations.AeCorrelatedProperties corrProps = combos
							.getPropertyCombinations(getMaxCorrelationCombinations());
					...
					case AeCorrelationCombinations.AeCorrelatedProperties.INITIATED_AND_JOIN:
						...
						// stop searching regardless of whether we found a match
						// since this strategy accounts for all possible
						// combinations of correlationSets
						done = true;
						break;
				...
				if (aInboundReceive.isCorrelated()) {
					for (Iterator iter = receives.iterator(); iter.hasNext()
						...
						if (queuedReceive.isCorrelated()
								&& queuedReceive.correlatesTo(aInboundReceive)) {
							// If the queued receive IS correlated, but it has
							// 'duplicate receives'
							// which prevent the correlation hash technique from
							// working.
							found = queuedReceive;
						}
					}
			...
			// If we DID find a match, then we must 'consume' it for it to be
			// official.
			else {
				...
				long journalId = consumeMessageReceiver(found, aInboundReceive,
						aAckCallback);

				// If we fail to consume the message receiver, then we need to
				// loop again to look
				// for another match.
	...
	/**
	 * This method is called to 'consume' a matched receive. A matched receive
	 * is consumed if it can be transferred from the QueuedReceive table to the
	 * Journal table. This is done by deleting it from the QueuedReceive table
	 * and subsequently adding the inbound receive to the journal. Note that the
	 * message receiver is deleted from the queued receive table by its group
	 * id. At the same time, any Alarms that have been queued with the same
	 * group id will be removed from the alarm queue table.
	 * 
	 * @param aMessageReceiver
	 * @param aInboundReceive
	 * @param aAckCallback
	 *            durable invoke message acknowledge callback.
	 * @return the journal id if consumed, or
	 *         <code>IAeProcessManager.NULL_JOURNAL_ID</code> if not consumed
	 * @throws AeStorageException
	 */
	protected long consumeMessageReceiver(
			AePersistedMessageReceiver aMessageReceiver,
			AeInboundReceive aInboundReceive,
			IAeMessageAcknowledgeCallback aAckCallback)
			throws AeStorageException {
		beginTransaction();
		try {
			...
			long rval = consumeMessageReceiverInternal(aMessageReceiver,
					aInboundReceive, aAckCallback);
	 ...
	 * @throws AeStorageException
	 */
	protected long consumeMessageReceiverInternal(
			AePersistedMessageReceiver aMessageReceiver,
			AeInboundReceive aInboundReceive,
			IAeMessageAcknowledgeCallback aAckCallback)
			throws AeStorageException {
		...
		try {
			// We consumed the message receiver only if we were the thread that
			// successfully
			// deleted it.
			if (aMessageReceiver.isConcurrent()) {
				associateAttachments(aInboundReceive, procId);

				journalId = getQueueStorageProvider().journalInboundReceive(
						procId, locId, aInboundReceive, connection);
				// do message ack callback.
				if (aAckCallback != null) {
					aAckCallback.onAcknowledge(null);
				}
			} else if (getQueueStorageProvider().removeReceiveObjectsInGroup(
					procId, groupId, locId, connection) > 0) {
				// Remove alarms in the group.
				getQueueStorageProvider().removeAlarmsInGroup(procId, groupId,
						connection);
	 ...
	 */
	protected void associateAttachments(AeInboundReceive aInboundReceive,
		...
		if (messageData != null && messageData.hasAttachments())
			AeEngineFactory
					...
					.associateProcess(messageData.getAttachmentContainer(),
							aProcessId);
	}

	/**
	 * Executes a search for each combination of property values until it finds
	 * a match.
	 * 
	 * @param aInboundReceive
	 * @param aCorrProps
	 * @throws AeStorageException
	 */
	protected AePersistedMessageReceiver findCorrelatedMatch(
			AeInboundReceive aInboundReceive,
			AeCorrelationCombinations.AeCorrelatedProperties aCorrProps)
			throws AeStorageException {
		AePersistedMessageReceiver found = null;
		List receives = null;
	 ...
	 * @throws AeStorageException
	 */
	protected AePersistedMessageReceiver findEngineCorrelatedMatch(
			AeInboundReceive aInboundReceive) throws AeStorageException {
		AePersistedMessageReceiver found = null;
		List receives = null;
	...
	/**
	 * @see org.activebpel.rt.bpel.server.engine.storage.IAeQueueStorage#removeReceiveObject(long,
	 *      int)
	 */
	public AeMessageReceiver removeReceiveObject(long aProcessId,
			int aMessageReceiverPathId) throws AeStorageException {
		try {
			AePersistedMessageReceiver rval = getQueueStorageProvider()
					.getReceiveObject(aProcessId, aMessageReceiverPathId);

			if (rval != null) {
				if (!getQueueStorageProvider().removeReceiveObjectById(
						rval.getQueuedReceiveId())) {
	...
	protected List<AePersistedMessageReceiver> getMatchingReceives(AeInboundReceive aInboundReceive,
		...
		if (aCorrelationMap != null) {
			correlatesHash = AeStorageUtil
					.getReceiveCorrelatesHash(aCorrelationMap);
		}

		List list = getQueueStorageProvider().getReceives(matchHash,
				correlatesHash);

		// If we get more than 1 receive, we might want to log that (hash
		// collision detected).
		// Only care about correlated collisions, since it's entirely possible
		// to get uncorrelated
		// collisions.
	 ...
	 */
	public AeMessageReceiverListResult getQueuedMessageReceivers(
	...
	/**
	 * @see org.activebpel.rt.bpel.server.engine.storage.IAeQueueStorage#storeAlarm(long,
	 *      int, int, int, java.util.Date)
	 */
	public void storeAlarm(long aProcessId, int aLocationPathId, int aGroupId,
			int aAlarmId, Date aDeadline) throws AeStorageException {
		getQueueStorageProvider().storeAlarm(aProcessId, aLocationPathId,
				aGroupId, aAlarmId, aDeadline);
	}

	/**
	 * @see org.activebpel.rt.bpel.server.engine.storage.IAeQueueStorage#removeAlarm(long,
	 *      int, int)
	 */
	...
	/**
	 * Removes an alarm from the storage. This version will use the given
	 * connection to execute the SQL statement.
	 * 
	 * @param aProcessId
	 ...
	 * @throws AeStorageException
	 */
	protected boolean removeAlarm(long aProcessId, int aLocationPathId,
			int aAlarmId, IAeStorageConnection aConnection)
			throws AeStorageException {
		return getQueueStorageProvider().removeAlarm(aProcessId,
				aLocationPathId, aAlarmId, aConnection);
	}

	/**
	 * @see org.activebpel.rt.bpel.server.engine.storage.IAeQueueStorage#removeAlarmForDispatch(long,
	 *      int, int, int)
	 */
	public long removeAlarmForDispatch(long aProcessId, int aGroupId,
			int aLocationPathId, int aAlarmId) throws AeStorageException {
		IAeStorageConnection connection = getCommitControlDBConnection();
		long journalId = IAeProcessManager.NULL_JOURNAL_ID;
		try {
			// First, remove any message receivers in the group.
			getQueueStorageProvider().removeReceiveObjectsInGroup(aProcessId,
					aGroupId, aLocationPathId, connection);
			// Next, remove the alarm
			if (removeAlarm(aProcessId, aLocationPathId, aAlarmId, connection)) {
				journalId = getQueueStorageProvider().journalAlarm(aProcessId,
						aGroupId, aLocationPathId, aAlarmId, connection);
				connection.commit();
			} else {
				connection.rollback();
			}
	 ...
	 */
	public void storeReceiveObject(AeMessageReceiver aReceiveObject)
			throws AeStorageException {
	...
	/**
	 * @param aLogCollisions
	 *            The logCollisions to set.
	 */

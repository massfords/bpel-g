...
public class AeDate {
	/**
	 * Indicates the date was parsed using a pattern which contained date and
	 * time.
	 */
	public final static int DATETIME = 0;

	/**
	 * Indicates the date was parsed using a pattern which contained only the
	 * date.
	 */
	...
	/**
	 * Parses and returns a Date object given a date and time as a string as
	 * well as the date format pattern.
	 * 
	 * @param aDateTime
	 *            date string
	 * @param aDateTimeFormatPattern
	 *            format pattern
	 * @return Parsed date or null if the string date format is not supported.
	 */
	public static Date parseDate(String aDateTime, String aDateTimeFormatPattern) {
		Date rVal = null;
		if (AeUtil.notNullOrEmpty(aDateTime)) {
			DateFormat dateFormat = AeUtil
					.notNullOrEmpty(aDateTimeFormatPattern) ? new SimpleDateFormat(
	...
	public static Date toDate(XMLGregorianCalendar aCal) {
		if (aCal == null)
			return null;
		else
			return aCal.toGregorianCalendar().getTime();
	}

	public static XMLGregorianCalendar toCal(Date aDate) {
		if (aDate == null)
			return null;
	...
	public static Date nextDay(XMLGregorianCalendar aCal) {
		return getNextDay(toDate(aCal));
	}
}
